...
- useCallback and useMemo can both be used for performance optimizations but carefully. They allow control of rerenders based on the provided dependency array.
- useMemo allows you to provide a function that returns any memoized value, usually for expensive computations or transformations. Returns the value.
- useCallback memoizes callback functions reducing recreation of that function on rerenders and optimizing component rendering. Returns the function itself.
- Can use the 'Performance' tab to record and profile webpage events.
- Wrap expensive static methods with useMemo and tie the method to the required function arguments as its dependencies
- ~16ms refers to a 60 frame rate
- Building the code in production makes it quicker, compared to development build, as the production build is for efficiency (minimizing files, etc.). Be sure to test the production build so you know what the user will truly experience.
- Recall: JavaScript is SINGLE threaded. No computations can run parallel.
- Web Workers can help run computations in parallel, preventing bottlenecks

...
- Less code is quicker
- Dynamic imports with lazy loading can be used on modules to load in. Use the React.lazy() however ensure you have a surrounding React.Suspense with the fallback attribute returning a fallback element
- All imports are cached, so the import can be explicitly called earlier, even with the lazy loading.
- This way you can stitch some earlier form of user interaction to dynamic loading of assets
- webpack has an API to start pre-loading assets and cacheing by adding to the document head, as soon as it is no longer busy.
  - import(/*webpackPrefetch: true */ ' ./ ')
- Careful you do not surround the entire family tree with the React.Suspense, so only the component that is loading, is replaced by the fallback
- Can use the 'Coverage' tab in the dev tools to see how much was actually run.
- Ensure you test the production builds when doing performance optimizations

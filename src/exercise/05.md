...
- useMemo can help stabilize values during equality comparison with React Context.
- The value referenced by a context provider, if changed, can cause rerenders by every component held by the context whenver the provider itself rerenders.
- Memoizing the value itself, keeps it constant and therefore all components reliant on it constant, while the provider may rerender (and the value stays the same if not changed) 
- Recall ~ Memo vs useMemo vs useCallback
  - Memo: To memoize entire COMPONENTS. Uses changes to internal props to determine rerender
  - UseMemo: A hook to memoize VALUES. Uses provided dependency array to determine rerender. Returns the value calculated as a result of the provided function.
  - useCallback: A hook to memoize FUNCTIONS. Uses provided dependency array to determine rerender. Returns the provided function itself.

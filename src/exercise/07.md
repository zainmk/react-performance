...
- React has an API for internal profiling of the application to test the user experience and speed in the production environment. Allows a way to assess the app before its in the client's hands.
- Many renders can occur at once, hence establishing a queue to hold the events and submitting it as a bulk request to the backend. 

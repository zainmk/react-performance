...
- You can purposely opt a component out of rerendering by using either React.PureComponent (class) or React.memo (functions). If the component unnecessarily rerenders because the parent component has re-rendered, this can force it to remain consistent.
- Reassign the function in a React.memo to limit its rerenders based on parent.
- Use the 'Performance' tab to profile the entire application and assess how long it takes to run the renders
- Second argument of React.memo() is the comparator function that takes (prevProps, nextProps) => and return true if no render required. Must define for all cases of prop changes and return accordingly.
- Object = React.memo(Object, (prevProps, nextProps) => true/false). True when no render required.
- Moving calculations higher in the tree can limit primitive types/values to the children, therefore limiting the rerenders for the children based on changes to that computation.

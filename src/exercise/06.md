...
- Consider the different methods of state management; Redux, Context, hooks, etc..., how does each method scale with the size of the app.
- A component with too much over-visibility of a variable can be detrimental as any change to the unused variables will trigger rerenders. Keep variables as tight and close to the componenets that require them without any additional visibility. -> Colocation
- Keeping contexts separate allows global visiblity when needed, but indepdendent updates per context keep from unnecessary rerenders.
- HOC (Higher Order Component) is a generic component that allows state splicing when the shared state is too cost heavy.
- Recoil is another state management library to use in React.
  - We can remove memoizations (React.memo) when having recoil manage the state changes for us.
  - Keeps state changes more closely tied and referenced to the component itself.
